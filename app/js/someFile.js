function someFunction() {
  jQuery(document).ready(function($){
    //store DOM elements
    var imageWrapper = $('.projects__list'),
      imagesList = imageWrapper.children('li'),
      contentWrapper = $('.descriptions'),
    ...
    //on mobile - close visible project when clicking the .close btn
    contentWrapper.on('click', '.close', function(){
      var closeBtn = $(this);
      if( !animating ) {
        animating = true;
        ...
        closeBtn.removeClass('is-scaled-up').one('webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend', function(){
          ...
          $('.projects').removeClass('content-block-is-visible');
    ...
    function updateBlock(n, device) {
      if( !animating) {
        ...
        if( device == 'mobile') {
          ...
          $('.projects').addClass('content-block-is-visible');
          contentWrapper.addClass('is-visible').one('webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend', function(){
            contentWrapper.find('.close').addClass('is-scaled-up');
            animating = false;
          });
        } else {
